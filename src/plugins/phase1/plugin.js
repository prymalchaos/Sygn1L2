...
export default {
  ...
  mount(root, api) {
          ...
          <div id="devPanel" style="display:${isDev ? "block" : "none"}; margin-top:16px;">
            ...
            <div style="margin-top:12px; border-top:1px solid rgba(215,255,224,0.12); padding-top:12px;">
              ...
              <div style="display:flex; gap:10px; flex-wrap:wrap; margin-top:10px;">
                ...
                <button id="wipeTarget" style="padding:10px; border-radius:10px;">Wipe save (user id)</button>
    ...
    if (isDev) {
      ...
      // Wipe save for target user id (uses same delete function but without deleting auth user)
      // If you haven't deployed a dedicated wipe function, this will error.
      root.querySelector("#wipeTarget").onclick = async () => {
        const user_id = targetUserIdEl.value.trim();
        if (!user_id) { setAdminOut("Enter a target user_id first."); return; }

        setAdminOut("Wipe save: requires Edge Function 'admin-user-wipe' (not yet created).");
      };
    ...
    // Battery-friendly repaint loop to match core tick
    render();
