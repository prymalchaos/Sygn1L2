...
function attachHoldPing(btn, doPing, getRate, canHold, onHoldChange) {
  ...
  const stop = () => {
    holding = false;
    btn.__holdActive = false;
    btn.__holdStopAt = Date.now();
    vis.holdPing = false;
...
<div class="p1-sensorsRow" style="display:flex; gap:10px; flex-wrap:wrap; align-items:stretch;">
            <div class="p1-panel p1-crt" style="flex: 1 1 340px; min-width: 260px;">
            ...
            <div class="p1-panel p1-crt" style="flex: 0 0 230px; min-width: 200px;">
            ...
            <div id="fatigueHud" class="p1-panel p1-crt" style="display:none; flex: 0 0 230px; min-width: 200px;">
              <div class="p1-row" style="justify-content:space-between;">
                <div class="p1-title">FATIGUE</div>
...
// visuals: spike the scope
      ...
      api.setState(st);
      api.save// Ping: tap + press-and-hold auto-ping
    const $ping = root.querySelector("#ping");

    // Tap ping (safe with hold). Ignore the synthetic click that often follows a long-press on iOS.
    const tapPing = (e) => {
      const justHeld = ($ping.__holdStopAt && (Date.now() - $ping.__holdStopAt) < 250);
      if ($ping.__holdActive || justHeld) return;
      doPing(false);
    };

    $ping.addEventListener("click", tapPing);
    $ping.addEventListener("touchend", (e) => { e.preventDefault(); tapPing(e); }, { passive: false });
